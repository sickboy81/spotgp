...
import { Search as SearchIcon, Filter, MapPin, ArrowUpDown, TrendingUp, Clock, Heart } from 'lucide-react';
...
export default function Search() {
    ...
    const filteredProfiles = useMemo(() => {
        ...
        if (searchQuery.trim()) {
            ...
            result = result.filter(p => {
                ...
                const desc = normalizeString((p as any).description || '');
                const city = normalizeString((p as any).city || '');
                const services = ((p as any).services || []).map((s: string) => normalizeString(s)).join(' ');
        ...
        if (activeFilters.priceMax) {
            ...
            result = result.filter(p => ((p as any).price || 0) <= maxPrice);
        }
        ...
        if (activeFilters.verifiedOnly) {
            result = result.filter(p => (p as any).verified === true);
        }

        if (activeFilters.gender && activeFilters.gender.length > 0) {
            result = result.filter(p => activeFilters.gender.includes((p as any).gender));
        }

        // IMPORTANTE: Cada anúncio aparece em APENAS UMA categoria
        if (activeFilters.category) {
            result = result.filter(p => (p as any).category === activeFilters.category);
        }
        ...
        const sorted = [...result].sort((a, b) => {
            switch (sortBy) {
                ...
                default:
                    ...
                    if ((a as any).verified && !(b as any).verified) return -1;
                    if (!(a as any).verified && (b as any).verified) return 1;
                    return ((b as any).views || 0) - ((a as any).views || 0);
            }
        });
    ...
    const saveCurrentSearch = () => {
        ...
        if (!savedSearches.includes(searchKey)) {
            ...
            localStorage.setItem('saphira_saved_searches', JSON.stringify(newSearches));
        }
    };

    useEffect(() => {
        const saved = localStorage.getItem('saphira_saved_searches');
        if (saved) {
    ...
    return (
        <>
            <SEOHead
                title="Buscar Perfis - ACOMPANHANTES AGORA"
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-5 lg:grid-cols-7 gap-2.5">
                                {filteredProfiles.map((profile) => {
                                    const mainImage = (profile as any).media?.find((m: any) => m.type === 'image')?.url || 
                                                     'https://via.placeholder.com/400x600?text=Saphira';
                                        ...
                                        >
                                            ...
                                            <div className="absolute bottom-0 left-0 right-0 p-2.5 z-20 translate-y-1 group-hover:translate-y-0 transition-transform duration-500">
                                                ...
                                                <div className="flex items-center justify-between text-white/90 text-[10px] font-medium">
                                                    <span className="truncate mr-1">{(profile as any).city}</span>
                                                    <span className="flex-shrink-0">R$ {(profile as any).price}</span>
                                                </div>
                                            </div>
                                        </Link>
                                    );
